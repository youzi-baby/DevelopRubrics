# Rubric Generation Chat Prompt

这个文件用于测试不同聊天框大模型对 `supply_chain_eval.py` 采购任务生成 AdaRubric `DynamicRubric` 的能力。
...
For a task: "Use the search API to find the top 3 suppliers of steel pipes in Germany, compare their prices, and recommend the cheapest option."

A good rubric might include dimensions like:
- SearchStrategyQuality (weight: 1.5): Whether the agent formulates effective search queries that cover the requirement space.
- DataExtractionAccuracy (weight: 1.5): Whether the agent correctly extracts and structures supplier data from API responses.
- ComparativeReasoningRigor (weight: 1.0): Whether the agent systematically compares options using consistent criteria.
- RecommendationJustification (weight: 1.0): Whether the final recommendation is logically derived from collected data.
